...
function ObjInlineActionMute() {
	...
	{
		// LD-8722: updated condition to avoild the error when the mute function is not available
		// however this mute is not working for the youtube player, have to raise a bug for this if its needed
		if(this.YTPlayer && typeof this.YTPlayer.mute === 'function')
			this.YTPlayer.mute();
	}
	else if (this.iType == 'vimeo')
	{
		this.VimeoPlayer.getVolume().then(function (data) {
...
function ObjInlineActionUnmute() {
	...
	{
		// LD-8722: updated condition to avoild the error when the unMute function is not available
		// however this unMute is not working for the youtube player, have to raise a bug for this if its needed
		if(this.YTPlayer && typeof this.YTPlayer.unMute === 'function')
			this.YTPlayer.unMute();		
	}
	else if (this.iType == 'vimeo')
	{
		if (!isNaN(THIS.prevVolume) && THIS.VimeoPlayer)
			THIS.VimeoPlayer.setVolume(THIS.prevVolume);
	}
}
...
p.addScript = ObjInlineAddScript
p.createPlayer = ObjInlineCreatePlayer
p.createVimeoPlayer = ObjInlineCreateVimeoPlayer
...
function ObjInlineActivate() {
  ...
  if (this.iType == 'vimeo')
  {
    if (is.VimeoScriptLoaded)
      this.createVimeoPlayer();
    else
    {
      var THIS = this;
      setTimeout(function () { THIS.createVimeoPlayer(); }, 500)
    }
  }
...
/* LD-5803: 
* Handles the inline translation group for objects, specifically addressing LD - 5803.
* This function moves the group of objects to a new position while considering individual object positions.
*/
function ObjInlineTransGrp( tOut, tNum, dur, x, y, objNameArr ) {
	...
	var THIS = this,
		...
		rightMost = 0,
		bottomMost = 0,
		wndPageDiv = GetCurrentPageDiv(),
		//rcdAttData = THIS.rcdGetAttData(),
		divMove, indexVal, refElemIndexVal;

	if (THIS.bMoveInProg)
		return;

	THIS.bMoveInProg = true;

	x = parseInt(x);
	y = parseInt(y);

	function unscalePos(pos) {
		// LD-8566 if we're not scaling to window, we don't need to unscale the position.
		// transformScale may have been set (i.e. != 1) when it should not.
		if (window.bScaleToWindow) {
			pos.top /= transformScale;
			pos.left /= transformScale;
		}
		return pos;
	}

	if (this.isGrp || this.isBtnGrp) {
		var arObjs = []; // work with a copy!;

		// make sure all move-able elements are in the arObjs array
		...
		// Get the top most and left post object's position in the group
		for (var i = 0; i < arObjs.length; i++) {
			var obj = arObjs[i];
			var pos = unscalePos(triv$(obj.div).position());

			var w = triv$(obj.div).width();
			var h = triv$(obj.div).height();
			topMost = Math.min(topMost, pos.top);
			leftMost = Math.min(leftMost, pos.left);
			rightMost = Math.max(rightMost, pos.left + w);
			bottomMost = Math.max(bottomMost, pos.top + h);
			indexVal = triv$(obj.div).css('z-index');
		}

		// Remember where to put back the object divs after the move
		if (arObjs) {
			refElemIndexVal = triv$(THIS.div).css('z-index');
		}
		// Safety check; divGrpMove should be gone at this point
		if (!THIS.div) {
			THIS.div = document.createElement('div');
			wndPageDiv.appendChild(THIS.div);
		}

		// Setup the move div
		divMove = THIS.div;
		triv$(divMove)
			.attr('id', THIS.name ? THIS.name : 'moveDiv')
			.css('position', 'absolute')
			.css('visibility', 'inherit')
			.css('top', topMost)
			.css('left', leftMost)
			.css('width', rightMost - leftMost)
			.css('height', bottomMost - topMost)
			.css('z-index', indexVal);
		THIS.div.x = leftMost;
		THIS.div.y = topMost;

		// Put object divs on the move div		
		for (var i = 0; i < arObjs.length; i++) {
			var obj = arObjs[i];

			// Insert placeholder div			
			var divPlaceholder = document.createElement('div');
			divPlaceholder.id = 'ph_' + obj.div.id;
			divPlaceholder.style.display = 'none';
			//console.log("obj.div.parentNode :: ", obj.div.parentNode);
			obj.div.parentNode.insertBefore(divPlaceholder, obj.div);
			obj.div.divPH = divPlaceholder;

			// Now move the object div to the move div			
			var pos = unscalePos(triv$(obj.div).position());
			triv$(obj.div).appendTo(divMove);
			triv$(obj.div)
				.css('top', pos.top - topMost)
				.css('left', pos.left - leftMost);
		}

		THIS.objLyr.transDone = function () {
			// Put the object divs back to their original place (get them off the bus)			
			var parPos = unscalePos(triv$(THIS.div).position());	// LD-8566 need to unscale the position

			for (var i = 0; i < arObjs.length; i++) {
				var obj = arObjs[i];
				var pos = unscalePos(triv$(obj.div).position());

				var divPlaceholder = obj.div.divPH;
				if (divPlaceholder) {
					divPlaceholder.parentNode.insertBefore(obj.div, divPlaceholder);
					divPlaceholder.parentNode.removeChild(divPlaceholder);
				}
				else {
					// This shouldn't happen. It is for safety
					console.error('jsWndOther.prototype.transGrp - we cannot find placeholder div after moving group. Placing object div at the end of list.');
					wndPageDiv.appendChild(obj.div);
				}

				var newLeft = pos.left + parPos.left;
				var newTop = pos.top + parPos.top;
				triv$(obj.div)
					.css('top', newTop + 'px')
					.css('left', newLeft + 'px');

				// LD-8565 need to set the ending object position after the group move
				obj.objLyr.x = newLeft;
				obj.objLyr.y = newTop;
				THIS.objLyr.hasMoved = true;
			}
			//triv$(THIS.div).remove(); // In LD, the group div exists in the DOM by default unlike LO, so not removing this
			//THIS.div = undefined;
			triv$(THIS.div)
				.css('z-index', refElemIndexVal)
				.css('height', '0px');
			THIS.bMoveInProg = false;
		};

		THIS.objLyr.doTrans(tOut, tNum, dur, null, divMove.x, divMove.y, x, y, 0, 0, 0);
	}
}
...
function ObjInlineAddScript(scriptURL, callBack)
{
	AddFileToHTML(scriptURL, 'script', callBack, null, getDisplayDocument());
}
...
function ObjInlineCreateVimeoPlayer()
{
	var THIS = this;
	if (is.VimeoScriptLoaded)
	{
		var vidSrc = THIS.url;
		if (THIS.bAutoStart)
			vidSrc += (vidSrc.indexOf('?') == -1 ? '?' : '&') + 'autoplay=1&muted=1';
		if (THIS.bLoop)
			vidSrc += (vidSrc.indexOf('?') == -1 ? '?' : '&') + 'loop=1';

		var vimeoIframe = getDisplayDocument().createElement('iframe');
		vimeoIframe.src = vidSrc;
		vimeoIframe.width = '100%';
		vimeoIframe.height = '100%';
		vimeoIframe.frameBorder = 0;
		if (THIS.bFullscreen)
			vimeoIframe.allow = "fullscreen";

		THIS.div.appendChild(vimeoIframe);

		if (window.trivPlayer)
			THIS.VimeoPlayer = trivPlayer.createVimeoPlayer(vimeoIframe);
		else
			THIS.VimeoPlayer = new Vimeo.Player(vimeoIframe);

		THIS.VimeoPlayer.on('play', function (event) {
			THIS.bPlaying = true;
		});
		THIS.VimeoPlayer.on('pause', function (event) {
			THIS.bPlaying = false;
		});
		THIS.VimeoPlayer.on('ended', function (event) {
			THIS.bPlaying = false;
			var fnOnDone = getDisplayWindow()[THIS.name + 'onDone'];
