...
function ObjMediaBuildMediaString(){
  ...
  else //If it not any of the cases above we can assume it is media element compatible
  {
	  ...
	  var meFeatures = ['playpause', 'progress', 'current', 'duration', 'tracks', 'volume'];
	  // LD-8394
	  if (this.bFullscreen)
		  meFeatures.push('fullscreen');
	  if (this.bSpeedControl)
		...
		this.playerOptions = { 
				...
				success:function(mediaElem, domObj){
					...
					if(mediaElem.player){
						...
						// LD-8429 make transparent background for webm video. 
						// Note: bShowCaptionBox below is for audio only.
						if (mediaElem.src && mediaElem.src.toLowerCase().endsWith('.webm'))
							triv$(".mejs__container", THIS.div).css('background', 'transparent');
						...
						if(THIS.bHasCaption && mediaElem.player.captionsButton)
						{
							...
							if (THIS.rgbTextClr) // LD-7851: Caption text color
								triv$(".mejs__captions-text", THIS.div).css("color", THIS.rgbTextClr);
							if (THIS.isAudio())
						...
						mediaElem.player.exitFullScreen = function(){
							if(THIS.isFullScreen)
							{
								...
								//LD-8343: No need to rebuild the layout for the full-screen exit
								//rebuildLayout();
								if(window.ReFlow)
									ReFlow();
							}
  ...
  else
  {
	  if(this.isVideo())
	  {
		 ...
		 //LD-5928 force transform scale to 1 for video as it causes issues with rollover when it is above 1 - the size of mediaElement is the one that is respected anyway
		  ...
		  if(this.source.indexOf(".mp4") >=0 || this.source.indexOf(".mpd") >=0 || this.source.indexOf(".m3u8") >=0 || this.source.indexOf(".webm") >= 0) {
			this.divInt += '<source  ' +
...
function ObjMediaOnHide() {
  ...
  // LD-8542 time rail shows for a fraction of a second when hiding the media
	var elemTmp = this.objLyr.ele;
	triv$('.mejs__controls', elemTmp).hide();

	this.objLyr.actionHide();

	setTimeout(function () { triv$('.mejs__controls', elemTmp).show(); }, 10);
}
...
function ObjMediaRemoveListeners(){
	var mePlayer = this.mediaPlayer;
	if (mePlayer && typeof (mePlayer.player) != 'undefined' && mePlayer.player.media) {
		mePlayer.player.media.removeEventListener('progress', mePlayer.progFunc);
		mePlayer.player.media.removeEventListener('timeupdate', mePlayer.timeFunc);
		mePlayer.player.media.removeEventListener('timeupdate', mePlayer.dispCap);
		mePlayer.player.media.removeEventListener('timeupdate', mePlayer.curFunc);
		mePlayer.player.media.removeEventListener('timeupdate', mePlayer.durFunc);
		mePlayer.player.media.removeEventListener('timeupdate', mePlayer.dispSlides);

		// LD-8413 the following code was from LO jsWndObj.prototype.destroyMEPlayer function:
		// close all server side connections
		mePlayer.pause();
		mePlayer.src = "";
		mePlayer.load();
		//			if (mePlayer.remove)  // Most common path
		//				mePlayer.remove();	// LO-6831 commented out code
		if (mePlayer.player && mePlayer.player.remove) {
			// IE11 w/ Flash enabled using MP4.h264, ME reports native pluginType (not flash)
			mePlayer.player.remove();
		}
		// LD-8555 we need to use this.div to make sure we have the right page div
		triv$('audio,video', this.div).remove();	// LO-6831 based on ME player doc
	}
}
