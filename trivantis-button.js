...
function ObjButtonSetDisable( bSet )
{
	...
	else
	{
		...
		else if(this.stateFills[this.baseState].stylePath && !bSet) // LD-8364, LD-8442
...
function ObjButtonBuild() {
  ...
  this.bIsWCAG = is.bWCAG;
	...
	if(this.hasOuterShadow && is.svg)
	{
		...
		if (xOffset == 0 && yOffset == 0) 
		{
			...
			adjustedXPos = this.x - this.outerShadowBlurRadius;
			adjustedYPos = this.y - this.outerShadowBlurRadius;
		}
		else
		{
	...
	// LD-8706: Ignoring this increment for center offset as it maintains the positioning as before.
	if (xOffset != 0 || yOffset != 0) 
		if(!this.name.indexOf("button")>-1)
			adjustedWidth +=3;
  ...
  this.div += '<' + this.divTag + ' id="'+this.name+'" ';
	...
	if (this.altName)
		this.div += 'title="' + this.altName + '" alt="" ';
	else
		this.div += 'title="" alt="" ';
  ...
  if (this.altName && !this.text)
		this.divInt += 'aria-label="' + this.altName + '"'; // LD-8663: Removed empty title attribute since its affecting the hover feature
...
function ObjSVGShadow(objWidth, objHeight, xOffset, yOffset, thisObj)
{
	...
	if(xOffset <= 0 || yOffset <= 0) // LD-8706: xOffset and yOffset value is 0 for center offset shadow 
	{
		if(xOffset < 0)
			svgImageTag += 'x = "' + (xOffset - thisObj.outerShadowBlurRadius) + '" '
		if(yOffset < 0)
			svgImageTag += 'y = "' + (yOffset - thisObj.outerShadowBlurRadius) + '" '
		if (xOffset == 0 && yOffset == 0) // LD-8706: adjusting x and y values for center offset shadow
		{
			svgImageTag += 'x = "' + (xOffset - thisObj.outerShadowBlurRadius) + '" '
			svgImageTag += 'y = "' + (yOffset - thisObj.outerShadowBlurRadius) + '" '
		}		
	...
	else if (xOffset == 0 && yOffset == 0) // LD-8706: adjusting x, y, width and height for center offset shadow
	{
		svgImageTag += '<image name="' + thisObj.name + 'Img" id="' + thisObj.name + 'Img" xlink:href = "' + (!thisObj.bOffPage ? thisObj.imgOffSrc : '') + '" preserveAspectRatio="none" ';

		svgImageTag += 'x = "';
		
		mapOffsetX = thisObj.outerShadowBlurRadius;
		svgImageTag += mapOffsetX + '" width = "' + thisObj.w + 'px"'
	
		mapOffsetY = thisObj.outerShadowBlurRadius;
		svgImageTag += 'y = "' + mapOffsetY + '" height = "' + thisObj.h + 'px"'
...
function ObjButtonGetCSS(){
	...
	if(this.hasOuterShadow && is.svg)
	{
		if (xOffset == 0 && yOffset == 0) // LD-8706: adjusting x, y, width and height for center offset shadow
		{
			// Center shadow: Add blur padding symmetrically
			adjustedWidth = this.w + (2 * this.outerShadowBlurRadius);
			adjustedHeight = this.h + (2 * this.outerShadowBlurRadius);
			adjustedXPos = this.x - this.outerShadowBlurRadius;
			adjustedYPos = this.y - this.outerShadowBlurRadius;
