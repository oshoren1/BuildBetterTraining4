...
function ObjLayer(id, pref, frame, divEle)
{
	...
	if (divEle)
	{
		...
		if(elemTidx) // LD-8770
		{
			// LD-7621: Check if the element has aria-label="Scrollable Content" and set tabindex="0" to make it focusable.
			if (triv$(elemTidx).attr('aria-label') === 'Scrollable Content') {
				triv$(elemTidx).attr('tabindex', '0');
			} else {
				triv$(elemTidx).attr('tabindex', '-1');
			}
			// LD-8724: Remove tabindex from text divs with empty content to fix the reading order when use the 'scroll to' action to skip through the content
			if (elemTidx.nodeType === 1 &&
				elemTidx.tagName == 'DIV' &&
				elemTidx.id && elemTidx.id.indexOf('text') === 0 &&
				(!elemTidx.textContent || elemTidx.textContent.trim() === '')) 
			{
				triv$(elemTidx).removeAttr('tabindex');
			} 
		}		
...
function ObjLayerClipTo(t, r, b, l)
{
	...
	try
	{
		// LD-8316: Try to set the CSS clip property of the current object based on transition type (this.tNum).
		// If the object has specific transition types (4, 5, 6, 7, 14, 16) and WCAG is enabled, apply the clip with '!important'.
		// Otherwise, set the clip property without '!important'.
		switch (this.tNum)
		{
			case 4:
			case 5:
			...
			case 14:
			case 16:
				if (is.bWCAG && this.mStyle == 1)
				{
					this.styObj.setProperty('clip', "rect(" + t + "px " + r + "px " + b + "px " + l + "px)", 'important');
					break;
				}
			default:
...
{
	// LD-8510 remove the listeners when we navigate away from the page to fix memory leak.
	if (!isSinglePagePlayerAvail()) {
		getDisplayWindow().myTop.removeEventListener("orientationchange", orientationChange, false);
		getDisplayWindow().removeEventListener("resize", changeSize, false);
		getDisplayWindow().myTop.removeEventListener("resize", changeSize, false);
	}
...
function ObjLayerSlideEnd()
{
	if (this.orgPos)
	{
		...
		var ele = (this.ele.tagName && this.ele.tagName != 'DIV' && this.theObj && this.theObj.div) ? this.theObj.div : this.ele; //LD-6819 LD-8301 jmw
	...
	this.tTrans = -1;
	if (this.transDone) this.transDone(); // LD-5803: Invoke the function to return the object divs to their original positions.
...
function changeSize()
{
	...
	if (previousDevice == is.clientProp.device && is.clientProp.device == "Desktop")
		return;
	//LD-8343: Check if the device has changed or if the orientation is different,
    //and ensure that there's a valid previousDevice before comparing it.
	if (previousDevice && (previousDevice != is.clientProp.device || previousOrientation.toLowerCase() != is.clientProp.orientation.toLowerCase()))
	{
		rebuildLayout();
		try
		{
...
function addSvgShadowFilter(name, width, height, direction, depth, opacity, red, green, blue, blurRadius, type, borderWidth)
{
	...
	if (xOffset <= 0 || yOffset <= 0 || borderWidth)
	{
		...
		if (xOffset <= 0)
		{
			// LD-8706: added borderWidth to fix the shadow cutoff issue with borders
			borderWidth = borderWidth ? borderWidth : 0;

			var xDisplacementPercentage = (((xOffset - blurRadius - borderWidth) / width) * 100).toFixed(5);
		...
		if (yOffset <= 0)
		{
			// LD-8706: added borderWidth to fix the shadow cutoff issue with borders
			borderWidth = borderWidth ? borderWidth : 0;
			var yDisplacementPercentage = (((yOffset - blurRadius - borderWidth) / height) * 100).toFixed(5);
...
function findFrame(win, frameName)
{
	...
	// Check if the window has frames
	// LD-8672: Add try/catch block to handle the frames cross origin domain issue
	// LD-8705: Add try/catch block to handle x-domain error
	if (win.length > 0)
	{
		try
		{
		...
		{
			// Access to cross-origin frame denied; ignore and continue
		}
...
function addClickMap(objWidth, objHeight, xOffset, yOffset, thisObj)
{
	...
	str += '>\n'
	if (xOffset == 0 && yOffset == 0) // LD-8706: Positioning the clickable map based on the shadow's center offset
		str += '<g opacity="0" transform="translate(' + thisObj.outerShadowBlurRadius + ',' + thisObj.outerShadowBlurRadius + ')">\n'; // Apply offset
	else
		str += '<g opacity="0">\n'	
...
function addImageMap(obj)
{
	...
	if (obj.altName == null) strMap += 'alt="">';  // LD-8540
	else strMap += 'alt="' + obj.altName + '">';		//echo bug 19523: Jaws is reading the alt tag for images with actions here
	strMap += '</map>\n';
...
function isObjInScrollView(obj, evType)
{
	...
	// LD-8567: Retrieving the page scale value
	var pageScale = 1;
	if ((!window.bTrivResponsive && window.bScaleToWindow))
		pageScale = getFillBrowserScale().scale; // Getting the fill browser scale
	else
		pageScale = getDisplayWindow().pageScale;

	var objTop = Math.round(obj.y * pageScale);		//triv$(obj.div).offset().top;
	var objHeight = Math.round(obj.h * pageScale);	//triv$(obj.div).height();
...
function lectoraPreInit()
{
	(window.myTop || window).lectoraActivating = true;
}

function lectoraPostInit()
{
	setTimeout(() => { (window.myTop || window).lectoraActivating = false; }, 1000);
}

// LD-7137: Checks if the device is a touch device. 
...
{
	if ((window.myTop || window).lectoraActivating)
		return;
...
///////////////////////////////////////////////////////////////////////////////////////////////////
// LD-8743:  START: Lectora console logger 
///////////////////////////////////////////////////////////////////////////////////////////////////

var u = undefined, n = null, t = true, f = false;

function LectoraLogger(levelOrLevels)
{
	var THIS = this;

	THIS.enabled = f;
	THIS.levels = {}; // attrs are components and values are log levels - only used if localStorage is NOT available

	if ( THIS.isStorageAvail() )
		this.levels.NOTE = "localStorage is available so log levels are not stored here";

	if ( typeof levelOrLevels == 'object' )
		THIS.levels = levelOrLevels;
	else if ( typeof levelOrLevels == 'string' )
		THIS.setAllLevels(parseInt(levelOrLevels));
	else if ( typeof levelOrLevels == 'number' )
		THIS.setAllLevels(levelOrLevels);
}

LectoraLogger.COMP_GEN			= 'general';
LectoraLogger.COMP_ACTIONS		= 'actions';

LectoraLogger.LEVEL_OFF			= 0 + 0;	// add + 0 to keep JSConstantsFinder from removing
LectoraLogger.LEVEL_SEVERE		= 1 + 0;	// add + 0 to keep JSConstantsFinder from removing
LectoraLogger.LEVEL_WARN		= 2 + 0;	// add + 0 to keep JSConstantsFinder from removing
LectoraLogger.LEVEL_INFO		= 3 + 0;	// add + 0 to keep JSConstantsFinder from removing
LectoraLogger.LEVEL_CFG			= 4 + 0;	// add + 0 to keep JSConstantsFinder from removing
LectoraLogger.LEVEL_FINE		= 5 + 0;	// add + 0 to keep JSConstantsFinder from removing
LectoraLogger.LEVEL_FINER		= 6 + 0;	// add + 0 to keep JSConstantsFinder from removing
LectoraLogger.LEVEL_FINEST		= 7 + 0;	// add + 0 to keep JSConstantsFinder from removing
LectoraLogger.LEVEL_ALL			= 99 + 0;	// add + 0 to keep JSConstantsFinder from removing

LectoraLogger.MODE_ERROR		= 1 + 0;	// add + 0 to keep JSConstantsFinder from removing
LectoraLogger.MODE_WARN			= 2 + 0;	// add + 0 to keep JSConstantsFinder from removing
LectoraLogger.MODE_LOG			= 3 + 0;	// add + 0 to keep JSConstantsFinder from removing

LectoraLogger.prototype.isEnabled = function()
{
	return this.enabled && window.console && console.log && console.warn && console.error;
};

LectoraLogger.prototype.enable = function()
{
	this.enabled = t;

	return this; // for chaining on constructor
};

LectoraLogger.prototype.disable = function()
{
	this.enabled = f;
};

LectoraLogger.prototype.isStorageAvail = function()
{
	return !!window.localStorage;
};

LectoraLogger.prototype.getStorageComponentKey = function(component)
{
	return 'Lectora:logLevel:' + component;
};

LectoraLogger.prototype.validateComponent = function(component)
{
	component = ( typeof component == 'string' ? component.toLowerCase() : LectoraLogger.COMP_GEN );

	var aCompNames = this.getAllComponentNames();

	for ( var i = 0; i < aCompNames.length; i++ )
	{
		if ( LectoraLogger[aCompNames[i]].toLowerCase() == component )
			return component;
	}

	return LectoraLogger.COMP_GEN;
};

LectoraLogger.prototype.getAllComponentNames = function()
{
	var aCompNames = [];

	for ( var key in LectoraLogger )
	{
...
LectoraLogger.prototype.parseLevel = function(level) // level can be 'ALL', 'WARN', etc OR '6' or 3
{
	var iLevel = parseInt(level);

	if ( isNaN(level) )
	{
		var levelFromName = this.nameToLevel(level);
		iLevel = ( levelFromName != u ? levelFromName : LectoraLogger.LEVEL_OFF );
	}
	else
	{
		if ( iLevel > LectoraLogger.LEVEL_FINEST && iLevel != LectoraLogger.LEVEL_ALL )
			iLevel = LectoraLogger.LEVEL_FINEST;

		if ( iLevel < (LectoraLogger.LEVEL_FINEST * -1) )
			iLevel = LectoraLogger.LEVEL_FINEST * -1;
	}

	return iLevel;
};

LectoraLogger.prototype.getLevel = function(component)
{
	// if ( !this.isEnabled() ) return LectoraLogger.LEVEL_OFF;

	component = this.validateComponent(component);

	try
	{
		var compLogLevel = LectoraLogger.LEVEL_OFF;

		if ( this.isStorageAvail() )
		{
			var logLevel = localStorage.getItem(this.getStorageComponentKey(component));

			if ( logLevel != u )
				compLogLevel = this.parseLevel(logLevel);
		}
		else
		{
			if ( this.levels && this.levels[component] != u )
				compLogLevel = this.parseLevel(this.levels[component]);
		}
	...
	return LectoraLogger.LEVEL_OFF;
};

LectoraLogger.prototype.setLevel = function(component, level)
{
	component = this.validateComponent(component);
	level = this.parseLevel(level);

	var sLevel = this.levelToName(level);

	if ( this.isStorageAvail() )
	{
		localStorage.setItem(this.getStorageComponentKey(component), sLevel);
	}
	else
	{
		if ( !this.levels )
		...
		this.levels[component] = sLevel;
	}

	return component + '=' + sLevel;
};

LectoraLogger.prototype.setLevels = function(levels)
{
	var aResults = [];

	if ( levels && typeof levels == 'object' )
	{
		for ( var component in levels )
			aResults.push(this.setLevel(component, levels[component]));
	}

	return aResults.join(', ');
};

LectoraLogger.prototype.setAllLevels = function(level)
{
	if ( typeof level == 'number' || typeof level == 'string' )
	{
		...
		for ( var i = 0; i < aCompNames.length; i++ )
			aResults.push(this.setLevel(LectoraLogger[aCompNames[i]], level));
	}

	return aResults.join(', ');
};

LectoraLogger.prototype.clearAllLevels = function()
{
	var aCompNames = this.getAllComponentNames();

	for ( var i = 0; i < aCompNames.length; i++ )
	{
		var component = LectoraLogger[aCompNames[i]];

		if ( this.isStorageAvail() )
		{
			localStorage.removeItem(this.getStorageComponentKey(component));
		}
		else
		{
			if ( this.levels && this.levels[component] != u )
				delete this.levels[component];
		}
	}
};

LectoraLogger.prototype.levelToName = function(level)
{
	for ( var key in LectoraLogger )
	{
		if ( key.startsWith('LEVEL_') )
		{
			if ( LectoraLogger[key] == level )
				return key.substr('LEVEL_'.length);
		}
	}
};

LectoraLogger.prototype.nameToLevel = function(name)
{
	return LectoraLogger['LEVEL_' + name.toUpperCase()];
};

LectoraLogger.prototype.displayLevels = function()
{
	var aLevels = [];
	var aCompNames = this.getAllComponentNames();
	...
	{
		var component = LectoraLogger[aCompNames[i]];
		var bExists = ( this.isStorageAvail() ? localStorage.getItem(this.getStorageComponentKey(component)) != u : this.levels && this.levels[component] != u );

		if ( bExists )
			aLevels.push(component + ":" + this.levelToName(this.getLevel(component)));
	}

	var sLevels = (this.isEnabled() ? 'ENABLED' : 'DISABLED') + (!this.isStorageAvail() ? ' [no storage]' : '') + ': ';

	sLevels += ( aLevels.length > 0 ? aLevels.join(', ') : 'no components set' );

	if ( window.console && console.log )
		console.log(sLevels);

	return aLevels.length;
};

LectoraLogger.prototype.isLoggable = function(component, level)
{
	if ( !this.isEnabled() )
		return f;

	component = ( component || LectoraLogger.COMP_GEN );
	level = this.parseLevel(level);

	try
	{
		var compLogLevel = this.getLevel(component);
		var isMatchExact = ( compLogLevel < 0 && Math.abs(compLogLevel) == level );
		var isMatchLE = ( compLogLevel > 0 && level <= compLogLevel );
		var isMatchAlways = ( compLogLevel == LectoraLogger.LEVEL_ALL || level == LectoraLogger.LEVEL_ALL );

		if ( isMatchExact || isMatchLE || isMatchAlways )
			return t;
	}
	catch ( eat )
	{
...
LectoraLogger.prototype.error = function(component, level, msg)
{
	return this.write(component, level, msg, LectoraLogger.MODE_ERROR);
};

LectoraLogger.prototype.warn = function(component, level, msg)
{
	return this.write(component, level, msg, LectoraLogger.MODE_WARN);
};

LectoraLogger.prototype.log = function(component, level, msg)
{
	return this.write(component, level, msg, LectoraLogger.MODE_LOG);
};

LectoraLogger.prototype.write = function(component, level, msg, mode)
{
	...
	try
	{
		component = ( component || LectoraLogger.COMP_GEN );
		level = this.parseLevel(level);
		mode = ( mode != u ? mode : LectoraLogger.MODE_LOG );

		if ( this.isLoggable(component, level) )
		{
			var fn = ( mode == LectoraLogger.MODE_ERROR ? console.error : mode == LectoraLogger.MODE_WARN ? console.warn : console.log );

			fn(this.now() + ' ' + this.levelToName(level) + ' ' + component.toUpperCase() + ' | ' + msg);
...
LectoraLogger.prototype.now = function()
{
	var pad = function(num, len) { return Array(len + 1 - num.toString().length).join('0') + num; };
	var d = new Date();
	var dt = pad(d.getMonth() + 1, 2) + '/' + pad(d.getDate(), 2);
	var tm = pad(d.getHours(), 2) + ':' + pad(d.getMinutes(), 2) + ':' + pad(d.getSeconds(), 2) + '.' + pad(d.getMilliseconds(), 3);

	return dt + ' ' + tm;
};

var lectoraLogger = new LectoraLogger();

///////////////////////////////////////////////////////////////////////////////////////////////////
// END: Lectora console logger
///////////////////////////////////////////////////////////////////////////////////////////////////
