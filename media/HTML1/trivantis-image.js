		  ...
		  if(this.ind && this.altName)
		  {
				if(UseHtmlImgTag(this) || !this.hasOuterShadow)
				{
					if(this.objLyr.theObjTag)
					{
						...
						{
							this.objLyr.theObjTag.alt = this.altName+"-"+trivstrSTATNS;
							this.objLyr.theObjTag.parentNode.title = this.altName+"-"+trivstrSTATNS;
							this.objLyr.theObjTag.setAttribute('aria-label', this.altName+" "+trivstrSTATNS);
						}
						else if(this.status == 'inprogress' )
						{
							this.objLyr.theObjTag.alt = this.altName+"-"+trivstrSTATIP;
							this.objLyr.theObjTag.parentNode.title = this.altName+"-"+trivstrSTATIP;
							this.objLyr.theObjTag.setAttribute('aria-label', this.altName+" "+trivstrSTATIP);
						}
						else
						{
							this.objLyr.theObjTag.alt = this.altName+"-"+trivstrSTATC;
							this.objLyr.theObjTag.parentNode.title = this.altName+"-"+trivstrSTATC;
							this.objLyr.theObjTag.setAttribute('aria-label', this.altName+" "+trivstrSTATC);
						}
					}
				...
				else
				{
					...
					if(this.status == "notstarted")
					{
						this.objLyr.shadowObj.setAttribute('alt', this.altName+"-"+trivstrSTATNS);
						this.objLyr.shadowObj.parentNode.parentNode.title = this.altName+"-"+trivstrSTATNS;
						this.objLyr.shadowObj.setAttribute('aria-label', this.altName+" "+trivstrSTATNS);
					}
					else if(this.status == 'inprogress' )
					{
						this.objLyr.shadowObj.setAttribute('alt', this.altName+"-"+trivstrSTATIP);
						this.objLyr.shadowObj.parentNode.parentNode.title = this.altName+"-"+trivstrSTATIP;
						this.objLyr.shadowObj.setAttribute('aria-label', this.altName+" "+trivstrSTATIP);
					}
					else
					{
						this.objLyr.shadowObj.setAttribute('alt', this.altName+"-"+trivstrSTATC);
						this.objLyr.shadowObj.parentNode.parentNode.title = this.altName+"-"+trivstrSTATC;
						this.objLyr.shadowObj.setAttribute('aria-label', this.altName+" "+trivstrSTATC);
					}
				}
		  }
	  }
...
function ObjImageBuild() {
	...
	this.div = '';
	var altTag = '';
	//LD-8663: Updated title attribute based on the altName availability for hover feature
	if( this.altName && !this.bUseSvgFile) altTag = ' title="'+this.altName+ '" alt="'+'"'
	else if( this.altName && this.bUseSvgFile && !is.firefox) altTag = ' title="'+this.altName+'" alt="'+'"'
	else if( this.altName && this.bUseSvgFile && is.firefox) altTag = ' title="'+this.altName+'" alt="'+'"'
	else if( this.altName != null ) altTag = ' title="'+this.altName+'" alt=""'

	if( this.bEmptyAlt && this.bUseSvgFile) altTag += ' aria-hidden ="true"';
	if( this.bEmptyAltImage ) altTag += ' aria-hidden ="true"'; // LD-8540
...
function ObjImageBuildSvg(){
	...
	if(this.hasOuterShadow)
	{	
		// LD-8706: adjusting x, y, width and height for center offset shadow (for center offset both xOffset & yOffset value will be '0' )
		if (xOffset == 0 && yOffset == 0)
		{
			adjustedWidth = this.w + (2 * this.outerShadowBlurRadius);
			adjustedHeight = this.h + (2 * this.outerShadowBlurRadius);
			adjustedXPos -= this.outerShadowBlurRadius;
			adjustedYPos -= this.outerShadowBlurRadius;
		}
		else
		{
	...
	if(this.hasOuterShadow)
	{
		...
		// LD-8706: Moved this line down to fix the tag order (open/close) issue
		...
		{	
			...
			else if (xOffset == 0 && yOffset == 0) // LD-8706: adjusting x value for center offset shadow 
			{
				mapOffsetX = this.outerShadowBlurRadius;
				this.divInt += mapOffsetX + '" width="' + this.w + 'px"';
			}
			...
			if (xOffset == 0 && yOffset == 0) // LD-8706: adjusting y value for center offset shadow 
			{
				mapOffsetY = this.outerShadowBlurRadius; 
				this.divInt += ' y="' + mapOffsetY + '" height="' + this.h + 'px"';
			}
...
function ObjShapeBuildSvg(){
	...
	if(this.hasOuterShadow)
	{
		if (xOffset == 0 && yOffset == 0) // LD-8706: adjusting x, y, width and height for center offset shadow
		{
			adjustedWidth = Math.ceil(this.w + (2 * this.outerShadowBlurRadius));
			adjustedHeight = Math.ceil(this.h + (2 * this.outerShadowBlurRadius));
			adjustedXPos -= this.outerShadowBlurRadius;
			adjustedYPos -= this.outerShadowBlurRadius;
		}
		else
	...
	if(this.hasOuterShadow)
	{
		// LD-8424: Added object (shapes with shadows) name to aria-label to fix the SR issue
		this.divInt += '<svg tabindex="-1" focusable="false" aria-label="' + this.altName +'" width="' + adjustedWidth + 'px" height="' + adjustedHeight + 'px"'
		...
		{	
			...
			else if (xOffset == 0 && yOffset == 0) // LD-8706: adjusting y value for center offset shadow 
			{
				mapOffsetY = this.outerShadowBlurRadius;
				this.divInt += ' y="' + mapOffsetY + '" height="' + this.h + 'px"';
			}
...
function ObjShapeBuildSpanText(name, mapOffsetX, mapOffsetY)
{
	...
	if(mapOffsetX != 0 || mapOffsetY != 0 )
		txtReturn += "left:"+ mapOffsetX + "px; top:" +mapOffsetY + "px; width:" + this.w + "px; height:" + this.h + "px;"; // LD-8706: Adjusted text positioning based on the object's width and height
	txtReturn += '"';
...
function ObjImageBuildSvgImgFill( mapOffsetX , mapOffsetY)
{
	...
	if(this.hasOuterShadow)
	{
		if (xOffset == 0 && yOffset == 0) // LD-8706: adjusting width and height for center offset shadow
		{
			adjustedWidth = Math.ceil(this.w + (2 * this.outerShadowBlurRadius));
			adjustedHeight = Math.ceil(this.h + (2 * this.outerShadowBlurRadius));
		}
...
function ObjImageGetCSS(){
	...
	if(  is.svg && ( !UseHtmlImgTag(this)  || this.bUseSvgFile || bHasEffects))
	{
		...
		else
		{
			if (xOffset == 0 && yOffset == 0) // LD-8706: adjusting width and height for center offset shadow
			{
				adjustedWidth = this.w + (1 * Math.abs(xOffset)) + (2 * this.outerShadowBlurRadius);
				adjustedHeight = this.h + (1 * Math.abs(yOffset)) + (2 * this.outerShadowBlurRadius);
			}
			else {
				adjustedWidth = this.w + (1 * Math.abs(xOffset)) + (2 * this.borderWeight) + this.outerShadowBlurRadius;
				adjustedHeight = this.h + (1 * Math.abs(yOffset)) + (2 * this.borderWeight) + this.outerShadowBlurRadius;
			...
			if (xOffset == 0 && yOffset == 0) // LD-8706: adjusting x, y and clip for center offset shadow
			{
				adjustedXPos -= this.outerShadowBlurRadius;
				adjustedYPos -= this.outerShadowBlurRadius;
				
				clipRect = 'clip: rect(' + 0 + 'px ' + adjustedWidth + 'px ' + adjustedHeight + 'px ' + 0 + 'px);';
			}
			else if(xOffset < 0 || yOffset < 0)
			{
				if(xOffset < 0 && yOffset >= 0)
